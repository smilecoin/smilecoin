...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // smilecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "smilecoin Signed Message:\n";
...
{
    // smilecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespanNEW = 60 ; // smileCoin: every 1 minute
static const int64 nTargetSpacing = 60; // smileCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // smilecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // smilecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1396310400;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 99943;

        if (fTestNet)
        {
            block.nTime    = 1394812278;
            block.nNonce   = 997879;
        }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("smilecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("smilecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static smilecoinMiner(CWallet *pwallet)
{
    printf("smilecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running smilecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("smilecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&smilecoinMiner, pwallet));
}
